...
template<int pos, int x, typename InpVec, typename OutVec = Vector<>>
struct insert;
...
template<int pos, int x, int... rest2>
struct insert<pos, x, Vector<>, Vector<rest2...>> {
    using type = Vector<rest2..., x>;
};

template<int pos, int x, int i1, int... rest1, int... rest2>
struct insert<pos,x,Vector<i1, rest1...>,Vector<rest2...>> {
    static_assert(pos > 0, "Cannot insert at position less than 0");
